...
public class LoginServiceTest {
  ...
  private static final String LO_EOE = "East of England";
  private static final String LO_KSS = "Kent Surrey and Sussex";
  ...
  public void createUserByTokenShouldCreateNewUserWithRolesAndDbcs() {
    ...
    GroupRepresentation eoeGroup = new GroupRepresentation();
    eoeGroup.setName(LO_EOE);
    GroupRepresentation kssGroup = new GroupRepresentation();
    ...
    when(keycloakAdminClientServiceMock.getUserGroups(any())).thenReturn(userGroups);
    ...
    Assert.assertTrue(capturedHeeUser.getDesignatedBodyCodes().contains("1-AIIDWT"));
    Assert.assertTrue(capturedHeeUser.getDesignatedBodyCodes().contains("1-AIIDR8"));
  ...
  public void shouldAddUserRolesByToken() throws Exception {
    ...
    HeeUser result = service.updateUserByToken(UPDATED_TOKEN);
  ...
  public void shouldAddDbcFromGroups() {
    ...
    heeUser.setRoles(roles);
    HashSet<String> roleNames = Sets.newHashSet("RVAdmin",
        "uma_authorization");
    List<Role> foundRoles = Lists.newArrayList(rvAdminRoleMock);
    heeUser.setDesignatedBodyCodes(Sets.newHashSet("1-AIIDWT","1-AIIDR8"));
    GroupRepresentation eoeGroup = new GroupRepresentation();
    eoeGroup.setName(LO_EOE);
    GroupRepresentation kssGroup = new GroupRepresentation();
    kssGroup.setName(LO_KSS);
