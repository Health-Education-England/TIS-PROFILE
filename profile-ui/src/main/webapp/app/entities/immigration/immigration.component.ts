...
export class ImmigrationComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		this.registerChangeInImmigrations();
	}
	...
	trackId(index: number, item: Immigration) {
		return item.id;
	}


	registerChangeInImmigrations() {
		this.eventSubscriber = this.eventManager.subscribe('immigrationListModification', (response) => this.loadAll());
	}
